    ...
    class GameViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun updateItem(
            ...
            itemView.item_winner_command_left_btn.setOnClickListener {
                ...
                bet?.also { gameListener.onCommandLeft("$refix $adapterPosition - $commandsName", it) }
            }
            itemView.item_winner_draw_btn.setOnClickListener {
                val bet = model.markets["MRFT"]?.lines?.get("NA")?.bets?.get("X")
                bet?.also {   gameListener.onIDraw( "$refix $adapterPosition - $commandsName", it)}
            }
            ...
            itemView.item_winner_command_right_btn.setOnClickListener {
                ...
                bet?.also { gameListener.onCommandRight("$refix $adapterPosition - $commandsName", it)}
            }
        }
    }
}